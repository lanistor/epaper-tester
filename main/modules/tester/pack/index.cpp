...
static void start_refresh_test() {
  ...
  left_rect.x      = 100;
  left_rect.y      = 100;
  left_rect.width  = epd_rotated_display_width() / 2;
  left_rect.height = 200;
  auto left_test   = new refresh_test_service(left_rect, 12);
  left_test->start();
}
...
void refresh_test_service::start_inner() {
  ...
  // 切换渲染位置
  if (this->current_index == count - 4) {
    lv_obj_set_pos(this->frame, area.x, area.y + area.height + 100);
  }
